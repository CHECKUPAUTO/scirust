//! The canonical tensor facade executed on a WGPU device, through the public
//! `scirust` entry point only.
//!
//! ```text
//! CanonicalProgram -> CanonicalSession -> WgpuReferenceAdapter
//!                  -> generated WGSL -> GPU (or software Vulkan) -> TensorND
//! ```
//!
//! Every import below is `scirust::…`: if a type the WGPU path needs were
//! missing from `scirust::tensor_canonical`, this file would not compile.
//!
//! # Running these
//!
//! Without a Vulkan/Metal/DX12/GL driver there is no adapter and each test
//! skips. Set `SCIRUST_REQUIRE_WGPU=1` to turn that skip into a failure — the
//! CI lavapipe job does, so a silent no-op there cannot masquerade as a pass.
...
use scirust::tensor_canonical::{
    ...
    CpuComputeAdapter, ReferencePlanRuntime, TensorND, WgpuReferenceAdapter,
};
...
#[test]
fn the_facade_runs_on_a_wgpu_device() {
...
#[test]
fn the_wgpu_and_cpu_backends_agree_on_a_multi_operation_program() {
    let Some(adapter) = adapter_or_skip()
    else
    {
        return;
...
#[test]
fn one_prepared_session_serves_several_wgpu_executions() {
    let Some(adapter) = adapter_or_skip()
    else
    {
        return;
...
#[test]
fn a_rejected_input_leaves_the_wgpu_session_usable() {
