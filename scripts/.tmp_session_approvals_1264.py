...
start = text.index("/// Interactive resolver for `Ask` decisions.\n")
end = text.index("/// Built-ins with no side effects outside observation are readers.", start)
replacement = r'''/// One operator response to an `Ask` decision.
...
impl PermissionGate {
    ...
    pub fn approve_tool_for_session(&self, tool: &str) -> Result<(), String> {
        ...
        self.session_approved_tools
            .write()
            .map_err(|_| "session approval memory is unavailable; refusing to mutate it".to_string())?
            .insert(key);
        Ok(())
    }
    ...
    pub fn is_tool_approved_for_session(&self, tool: &str) -> Result<bool, String> {
        ...
        Ok(self
            .session_approved_tools
            .read()
            .map_err(|_| "session approval memory is unavailable; refusing remembered approval".to_string())?
            .contains(&key))
    }
...
module = Path("scirust-sciagent/src/agentic/mod.rs")
mod_text = module.read_text()
old = """pub use permission::{\n    PermissionDecision, PermissionGate, PermissionPolicy, PermissionRule, ToolApprover,\n};\n"""
new = """pub use permission::{\n    ApprovalChoice, PermissionDecision, PermissionGate, PermissionPolicy, PermissionRule,\n    ScopedToolApprover, ToolApprover,\n};\n"""
if old not in mod_text:
    raise SystemExit("permission re-export anchor not found")
module.write_text(mod_text.replace(old, new, 1))
