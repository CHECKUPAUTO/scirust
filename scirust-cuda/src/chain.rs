...
// Deterministic two-stage sum-of-squares for global grad clipping. Stage 1 writes
...
impl CudaChain {
    ...
    pub fn rms_norm_gain_backward(&self, x: &CudaMatrix, dy: &CudaMatrix, eps: f32) -> CudaMatrix {
        ...
        let (rows, cols) = (x.rows, x.cols);
        let mut inv_rms = self
            .stream
            .alloc_zeros::<f32>(rows)
            .expect("cuda alloc inv_rms");
        let (rows_a, cols_a, eps_a) = (rows, cols, eps);
    ...
    /// Mean next-token cross-entropy computed on the device. Only `rows` fp32 loss
    ...
    pub fn global_grad_sumsq(&self, grads: &[&CudaMatrix]) -> CudaF32 {
        ...
        for g in grads
        {
            ...
            unsafe {
                stage1.launch(cfg1).expect("launch sumsq_partials_kernel");
            }
            ...
            unsafe {
                stage2.launch(cfg2).expect("launch sumsq_finish_kernel");
            }
        }
