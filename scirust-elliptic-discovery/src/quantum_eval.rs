...
impl QuantumIsogenyEvaluator {
    ...
    pub fn evaluate_shor_security(&self, point_order: u64) -> ShorAssessment {
        ...
        // Note: apply_2qubit_gate(q, gate, max_chi) operates on adjacent qubits q and q+1.
        // The third parameter '4' is the bond dimension cap max_chi, NOT a qubit index.
        ...
        let success_probability = if norm_sq > 0.0
        {
            max_amp_sq / norm_sq
        }
        else
    ...
    pub fn evaluate_isogeny_resistance(&self) -> IsogenyAssessment {
        // We model a 4-qubit quantum walk on a local isogeny graph
        let num_qubits = 4;
        let mut mps = Mps::zero(num_qubits);
        ...
        mps.apply_2qubit_gate(2, &gates::CZ, 4);
    ...
    pub fn model_hybrid_pipeline(&self, key_parameter: u32) -> f32 {
        ...
        let collapse_factor =
            shor.success_probability * (1.0 - (isogeny.walk_amplitude_entropy / 3.0));
