...
- **Prospective dynamic-information analysis** (`scirust-tdi`) — an exact, deterministic toolkit (ported from the *TDI* research project) asking whether the **structure of accessible futures** carries predictive information that scalar summaries such as Shannon entropy do not preserve: **exact finite-state dynamics** with arbitrary-precision **rational** probabilities (no floating-point rounding), **future-structure descriptors** (future-block / branching-path / branching-state distributions and the intervention-conditioned **`distribution_overlap`**), and **honest baselines** (Shannon block entropy, an orbital baseline, perturbation-recovery analysis). Its 40 exact tests carry over verbatim. Honest about its own limits: within the tested synthetic families the overlaps beat an increasingly strong exact control ladder — contraction descriptors, spectral moments `s₂, s₃, s₄`, the **literal** spectral gap and ε-mixing time — across four generator families and three widths, and the advantage *grows* under a degree-2 nonlinear model; but TDI-1's own signal was fully subsumed by the orbital baseline (incremental gain `0`), no universal law is claimed, and **transportable calibration is a measured failure** with two label-free repairs preregistered and refuted (TDI-6.6, TDI-6.7). What survives a domain change is rank ordering. The deterministic, exact machinery and the preregistration discipline are what transfer; the full upstream record is copied verbatim under [`docs/tdi-upstream/`](docs/tdi-upstream/). The companion `scirust-tdi-bench` re-homes the reproducible evaluation harness (deterministic scans and counterexample search, untouched holdouts, ridge models, and a **deterministic bootstrap** for confidence intervals, with frozen preregistration constants) as ten deterministic benchmark binaries covering the TDI-1, TDI-2 and TDI-5.1 evaluators; the later confirmatory evaluators (TDI-5.2 … TDI-6.8) remain upstream, and their reports are copied under [`docs/tdi-upstream/`](docs/tdi-upstream/).
...
scirust-tdi-bench/     Reproducible TDI benchmarks: deterministic scans, counterexamples, untouched holdouts, ridge models, deterministic bootstrap CIs, frozen preregistration constants (ten benchmark binaries over scirust-tdi, covering TDI-1/2/5.1)
