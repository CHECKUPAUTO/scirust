...
if [ -z "$REPORT" ]; then
    REPORT="$(
        find "$WORK_ROOT/results" -type f \
            -name dream_repeated_crossover_gsm8k_report.json \
            -printf '%T@ %p\n' 2>/dev/null |
            sort -nr |
            head -n 1 |
            cut -d' ' -f2-
...
[ -n "$REPORT" ] && [ -f "$REPORT" ] || fail "rapport crossover répété introuvable"

OUTPUT="$(dirname "$REPORT")/dream_repeated_crossover_gsm8k_summary.json"
printf 'Résumé sans recalcul GPU: %s\n' "$REPORT"
python3 \
    "$SCIRUST_RUN/experiments/elastic-cache-policy/jetson/summarize_repeated_crossover.py" \
    --report "$REPORT" \
    --output "$OUTPUT"
