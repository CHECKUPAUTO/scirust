...
OUTPUT="$(dirname "$REPORT")/dream_guarded_independent_quality_diagnostic.json"
printf 'Diagnostic sans recalcul GPU: %s\n' "$REPORT"
python3 \
    "$SCIRUST_RUN/experiments/elastic-cache-policy/jetson/diagnose_guarded_independent_quality.py" \
    --report "$REPORT" \
    --output "$OUTPUT"
