...
fn feature_functions(table: &Arc<Vec<[f64; RUNTIME_FEATURES]>>) -> Vec<Box<FeatureFn>> {
...
pub fn fit_sequential_risk(
    ...
    let prior = ((unsafe_count as f64 + 0.5) / (total as f64 + 1.0)).clamp(1e-6, 1.0 - 1e-6);
    ...
    for epoch in 0..epochs
    {
        ...
        {
            return Err(format!(
                "CRF optimization became non-finite at epoch {epoch}"
            ));
        }
    }
