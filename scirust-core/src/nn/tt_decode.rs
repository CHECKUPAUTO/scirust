...
impl<'a> TwoCoreTtDecodePlan<'a> {
    ...
    pub fn parameter_count(&self) -> usize {
        self.core0
            ...
            .saturating_add(self.bias.map_or(0, |bias| bias.data.len()))
    }
        ...
        for out0 in 0..self.out0
        {
            for out1 in 0..self.out1
            {
                ...
                for rank in 0..self.rank
                {
                    ...
                    for i1 in 0..self.in1
                    {
                        sum += scratch[scratch_base + i1]
                            * self.core1.data[core1_base + i1 * self.out1 + out1];
                    }
                }
...
mod tests {
    ...
    fn dense_reference(layer: &TTLinear, input: &[f32]) -> Vec<f32> {
        ...
        let mut output = layer
            .bias
            .as_ref()
            .map_or_else(|| vec![0.0; layer.out_features], |bias| bias.data.clone());
        for output_col in 0..layer.out_features
        {
            for input_row in 0..layer.in_features
            {
                output[output_col] +=
                    input[input_row] * weight.data[input_row * layer.out_features + output_col];
            }
        }
        output
    }
