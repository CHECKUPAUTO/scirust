...
- `Min`/`Max` call Rust `f32::min/max` or `f64::min/max`. Rust's NaN and signed-
  zero behaviour is therefore the elementwise contract. `Clamp(x,lo,hi)` is
  evaluated exactly as `x.min(hi).max(lo)` in that order; the verifier checks
  types/shapes but does not require `lo <= hi`.
...
- maximum starts at negative infinity and replaces the accumulator only when
  the incoming element is strictly greater;
- minimum starts at positive infinity and replaces it only when strictly less;
- mean performs the same ordered sum, then one native division by the reduced
  count.

Consequently ties in reduce-min/max retain the first encountered value,
including its signed-zero bit. Under `AllowNonFinite`, NaNs do not satisfy the
strict comparison and are ignored by reduce-min/max; an all-NaN maximum/minimum
therefore leaves the respective infinity identity. Default policies reject NaN
external inputs, so this edge is observable only through explicitly permissive
research execution or a computed NaN (which `FiniteOutputs` rejects at its
producer). Empty sum/product are defined by their identities. Empty
...
change the sign of zero according to native IEEE rules.
...
Preserve evaluation structure and IEEE-observable distinctions: operation
order, signed zero, NaN/infinity behaviour, and fused versus unfused rounding.
Allowed rules are exact structural rules: finite-result constant evaluation of
the same opcode, Boolean identities, `Select(mask,v,v)`, Boolean/compare operand
normalization where exact, double Boolean negation, exact CSE, stable root
renumbering, and DCE. Floating add/mul/min/max/dot operands are not reordered.
...
| `min(x,x)`, `max(x,x)` | no | yes | yes | finite-domain source |
| `-(-x) -> x` | no | yes | yes | finite-domain source |
| reorder float commutative operands | no | yes | yes | finite active operands; no reassociation |
