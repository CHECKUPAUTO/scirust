...
|-----------|--------------------------|--------------------------|-----|-----------------|--------------------|------------|------------|
| Approval Ask/Never | `ApprovalPolicy` in session events, last wins | `ApprovalPolicy::Ask/Never`, shared Arc<RwLock>, Never fails closed | ApprovalService currently also owns a separate policy value; authoritative single-source wiring remains TODO | divergent policy copies could present inconsistent supervision state | TODO atomic policy-source PR | #1270/#1271 | divergence tests + CI |
...
| model-facing policy context | runtime-context snapshot, live switch notices | PermissionGate has durable/shared policy; ApprovalService has a separate policy copy | single authoritative effective-policy source and live context propagation still TODO | avoids Ask/Never disagreement between enforcement and presentation | TODO atomic policy-source PR | #1270/#1271/#1274 | deliberate divergence test |
