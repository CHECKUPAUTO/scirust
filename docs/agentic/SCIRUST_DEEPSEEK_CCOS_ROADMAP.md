...
| H-5 | Kernel-enforced resource governance | OPEN PR | `feat/sciagent-real-resource-enforcement` + `feat/cgroupv2-resource-backend` | per-process `RLIMIT_FSIZE` plus deny-all INET egress via seccomp-BPF/bwrap; tree-wide memory/process/CPU now enforced through a throwaway cgroup v2 subgroup (`memory.max`/`pids.max`/`cpu.max`) when the hierarchy is writable — non-escapable for an unprivileged child; GPU caps, wall-time-as-cgroup and host allow-lists still refuse rather than approximate |
...
| request/cancellation race | cancellation can win asynchronously; late result discarded | answerer dispatched on a detached worker; the waiter polls the token (20 ms) so cancellation interrupts even a blocked synchronous `answer()`; first resolution wins and a detached late outcome can never add a second one; a worker panic resolves Unavailable | none at service level | a blocked human approver can no longer pin a session past its cancellation | this branch | #1274 | blocked-answerer interruption test, panicking-answerer fail-closed test, late-result discard preserved |
...
| enterprise budgets/egress | n/a | fail-closed governance wired through `ToolRuntime::execute_governed`: inherited `RLIMIT_FSIZE`, deny-all INET egress via seccomp/bwrap, and tree-wide memory/process/CPU via cgroup v2 subgroups created per governed execution and removed on drop | GPU caps and per-host egress allow-lists still refuse rather than approximate; wall time remains the runtime kill deadline (kernel-side wall ceiling would need a supervisor process) | declared limits must correspond to non-escapable enforcement | cgroup branch | #1280 | live tests: SIGXFSZ via RLIMIT_FSIZE, bash `/dev/tcp` blocked by seccomp, `pids.max` trips a forking child, capability tests mirror the probed hierarchy |
| enterprise correlated audit | n/a | `FileEnterpriseAuditTrail` JSONL SHA-256 chains with optional per-segment rotation (`with_rotation`, genesis-rooted fresh chain per segment, global sequence continuity), advisory flock across processes on a stable `.lock` file, verified on every read + `EnterpriseAuditSink`; `DeepSeekBridge::with_enterprise_audit` emits correlated executed/failed/rejected calls | approval-side stores (policy/audit) still single-chain without rotation; live switch notices to the model remain harness-side | restart preserves the trail across segments; concurrent agent processes serialize through the lock; audit refusal still prevents an unaudited success from reaching the model | rotation branch | #1325 | rotation/restart/tamper/concurrency tests; bridge emission + fail-closed sink tests |
