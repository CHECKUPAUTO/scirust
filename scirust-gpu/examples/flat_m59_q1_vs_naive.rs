//! M59 paired resident-attention benchmark: SciRust's existing naive multi-dispatch
//! WGPU composition versus FLAT M58's opt-in Q1 vec4 MHA candidate.
//!
//! The two paths share one SciRust-owned WGPU context and use separate resident
//! Q/K/V buffers populated from identical bytes. Upload and readback are outside
//! the timed region. The candidate route is checked explicitly before timing.
//! This benchmark emits measurement evidence only and makes no generic speedup claim.
...
mod enabled {
    ...
    use flat_attention::{
        ...
        GroupedForwardKernelVariant, GroupedForwardPass, WgpuGroupedForwardPipeline,
    };
    ...
    fn run_flat(
        ...
        let mut encoder = ctx
            ...
            .create_command_encoder(&wgpu::CommandEncoderDescriptor {
                label: Some("scirust-m59-flat-q1-forward"),
            });
        pipeline.encode(
            ctx.device(),
            &mut encoder,
    ...
    pub fn run() -> Result<(), Box<dyn Error>> {
        let seq_len = env_usize("SCIRUST_M59_SEQ_LEN", DEFAULT_SEQ_LEN);
        let head_dim = env_usize("SCIRUST_M59_HEAD_DIM", DEFAULT_HEAD_DIM);
        let warmups = env_usize("SCIRUST_M59_WARMUPS", DEFAULT_WARMUPS);
        let repeats = env_usize("SCIRUST_M59_REPEATS", DEFAULT_REPEATS);
        ...
        let pipeline = WgpuGroupedForwardPipeline::with_q1_vec4_mha(ctx.device(), true)?;
        ...
        let selected = pipeline.kernel_variant_for_shape(shape);
        if selected != GroupedForwardKernelVariant::Q1Vec4Mha {
            return Err(format!(
        ...
        let q_flat = flat_input_buffer(&ctx, &q, "scirust-m59-flat-q");
        let k_flat = flat_input_buffer(&ctx, &k, "scirust-m59-flat-k");
        let v_flat = flat_input_buffer(&ctx, &v, "scirust-m59-flat-v");
        ...
        println!(
            "adapter,backend,causal,seq_len,head_dim,warmups,repeats,naive_median_us,naive_p95_us,flat_fresh_median_us,flat_fresh_p95_us,flat_reused_median_us,flat_reused_p95_us,naive_over_flat_fresh,naive_over_flat_reused,naive_parity_max_abs,flat_parity_max_abs,flat_route,performance_claim"
        );
        ...
        for causal in [false, true] {
            ...
            assert_close("FLAT Q1 LSE", &flat_host[layout.lse_offset()..], &expected.lse)?;
            ...
            println!(
                "{},{},{},{},{},{},{},{:.3},{:.3},{:.3},{:.3},{:.3},{:.3},{:.6},{:.6},{:.8},{:.8},q1_vec4_mha,none",
