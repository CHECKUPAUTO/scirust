...
pub fn cscv_probability_of_backtest_overfitting(
    ...
    if slices < 2 || slices % 2 != 0 || slices > observations {
    ...
    for in_sample_slices in assignments {
        ...
        let worse = out_sharpes
            .iter()
            .enumerate()
            .filter(|(index, score)| **score < selected_oos || (**score == selected_oos && *index > selected_strategy))
            .count();
...
mod tests {
    ...
    fn cscv_detects_noisy_selection_instability() {
        ...
        assert!(report.splits.iter().all(|split| split.normalized_oos_rank > 0.0));
    }
}
