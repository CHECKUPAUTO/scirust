...
//! Spot and perpetual books must also satisfy explicit freshness and timestamp
//! alignment constraints before a scenario can be marked positive.
...
pub struct BasisScenarioConfig {
    ...
    /// Evaluation timestamp for entry-book freshness checks.
    pub now_ts_ms: i64,
    /// Maximum permitted age of either entry book.
    pub max_quote_age_ms: u64,
    /// Maximum permitted timestamp skew between spot and perpetual books.
...
fn timestamp_constraints(
    ...
    now_ts_ms: i64,
    max_age_ms: u64,
    from_future: BasisConstraint,
    stale: BasisConstraint,
    constraints: &mut Vec<BasisConstraint>,
) {
    if ts_ms > now_ts_ms
    {
...
pub fn analyze_basis_scenario(
    ...
    timestamp_constraints(
        ...
        BasisConstraint::SpotQuoteFromFuture,
        BasisConstraint::SpotQuoteStale,
        &mut constraints,
    );
    timestamp_constraints(
        perp_book.ts_ms,
        cfg.now_ts_ms,
        cfg.max_quote_age_ms,
        BasisConstraint::PerpQuoteFromFuture,
        BasisConstraint::PerpQuoteStale,
        &mut constraints,
    );
    let timestamp_skew_ms = spot_book.ts_ms.abs_diff(perp_book.ts_ms);
    if timestamp_skew_ms > cfg.max_leg_skew_ms
    {
        constraints.push(BasisConstraint::LegTimestampSkew);
    }
    ...
    let blocked = constraints.iter().any(|constraint| {
        matches!(
            ...
            BasisConstraint::SpotDepthInsufficient
                ...
                | BasisConstraint::SpotQuoteStale
                | BasisConstraint::PerpQuoteStale
                | BasisConstraint::LegTimestampSkew
...
mod tests {
    ...
    fn insufficient_depth_blocks_positive_scenario() {
        ...
        assert!(
            report
                .constraints
                .contains(&BasisConstraint::SpotDepthInsufficient)
        );
    }
    ...
    fn stale_or_skewed_entry_books_block_positive_scenario() {
        ...
        assert!(
            report
                .constraints
                .contains(&BasisConstraint::LegTimestampSkew)
        );
    }
    ...
    fn future_entry_book_blocks_positive_scenario() {
        ...
        assert!(
            report
                .constraints
                .contains(&BasisConstraint::SpotQuoteFromFuture)
        );
    }
    ...
    fn missing_collateral_is_reported_but_not_falsely_called_insufficient() {
        ...
        assert!(
            report
                .constraints
                .contains(&BasisConstraint::PerpCollateralUnverified)
        );
        assert!(report.scenario_positive);
    }
    ...
    fn insufficient_collateral_blocks_scenario() {
        ...
        assert!(
            report
                .constraints
                .contains(&BasisConstraint::InsufficientPerpCollateral)
        );
    }
    ...
    #[test]
    fn repeated_scenario_is_bit_reproducible() {
        let spot = book(990, 99.0, 100.0);
        let perp = book(995, 103.0, 104.0);
        let config = cfg(BasisDirection::CashAndCarry);
        let a = analyze_basis_scenario(&spot, &perp, &config).unwrap();
        let b = analyze_basis_scenario(&spot, &perp, &config).unwrap();
        assert_eq!(a, b);
    }
}
