...
pub fn parkinson_volatility(
    ...
    for (i, value) in out.iter_mut().enumerate().skip(period - 1)
    {
        let mut sum = 0.0f32;
        let mut valid = true;
        for j in (i + 1 - period)..=i
        ...
        if valid
        {
            ...
            *value = (variance.max(0.0) * periods_per_year).sqrt();
        }
    }
    out
}
...
pub fn garman_klass_volatility(
    ...
    for (i, value) in out.iter_mut().enumerate().skip(period - 1)
    {
        ...
        if valid
        {
            ...
            *value = (variance * periods_per_year).sqrt();
        }
    }
    out
}
