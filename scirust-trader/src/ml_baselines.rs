...
impl RidgeLinearRegressor {
    pub fn fit(x: &[Vec<f32>], y: &[f32], cfg: LinearFitConfig) -> Result<Self, BaselineError> {
...
type SplitCandidate = (f32, usize, f32, Vec<usize>, Vec<usize>);
...
fn build_tree(
    ...
    if depth >= cfg.max_depth
        || indices.len() < cfg.min_leaf.saturating_mul(2)
        || sse(y, indices) <= 1e-12
    {
    ...
    let Some((_, feature, threshold, left_indices, right_indices)) = best
    else
    {
        return RegressionTreeNode::Leaf { value: leaf_value };
    };
...
impl RandomForestRegressor {
    pub fn fit(x: &[Vec<f32>], y: &[f32], cfg: RandomForestConfig) -> Result<Self, BaselineError> {
        let width = validate_xy(x, y)?;
        if cfg.trees == 0
            || cfg.tree.min_leaf == 0
...
impl GradientBoostedTreesRegressor {
    pub fn fit(x: &[Vec<f32>], y: &[f32], cfg: GradientBoostConfig) -> Result<Self, BaselineError> {
        let width = validate_xy(x, y)?;
        if cfg.stages == 0
            || cfg.tree.min_leaf == 0
            || !cfg.learning_rate.is_finite()
...
impl AutoregressiveRegressor {
    pub fn fit(series: &[f32], lags: usize, cfg: LinearFitConfig) -> Result<Self, BaselineError> {
...
mod tests {
    ...
    fn seeded_random_forest_is_reproducible() {
        ...
        assert_eq!(
            a.predict(&[2.5, 3.0]).to_bits(),
            b.predict(&[2.5, 3.0]).to_bits()
        );
    }
    ...
    fn autoregression_is_deterministic() {
        ...
        assert_eq!(
            a.predict_next(&series).to_bits(),
            b.predict_next(&series).to_bits()
        );
    }
}
