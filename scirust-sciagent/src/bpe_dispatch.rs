...
impl VersionedBpeTokenizer {
    ...
    /// Ordered canonical merge table when this tokenizer has canonical BPE
    /// semantics. Legacy artifacts deliberately expose no synthetic rank table:
    /// their provenance is bound to the exact artifact bytes instead.
    pub fn canonical_ordered_merges(&self) -> Option<&[(TokenId, TokenId, TokenId)]> {
        match self {
            Self::Legacy(_) => None,
            Self::Canonical(tokenizer) => Some(tokenizer.ordered_merges()),
        }
    }
...
impl fmt::Display for BpeDispatchError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::UnknownMergeSemantics(value) => write!(f, "unknown BPE merge semantics `{value}`"),
