...
fn bubblewrap_profile_args(mode: SandboxMode, root: &Path) -> Vec<OsString> {
    let mut args = [
        "--ro-bind",
        "/",
        "/",
...
fn bubblewrap_command(
    ...
    command
        .args(bubblewrap_profile_args(mode, root))
        .arg("--chdir")
        .arg(cwd)
        .arg("--")
        .arg(program)
...
mod tests {
    ...
    fn read_only_profile_matches_harness_mount_contract() {
        let args = bubblewrap_profile_args(SandboxMode::ReadOnly, Path::new("/workspace"))
            .into_iter()
    ...
    fn workspace_write_profile_adds_only_private_tmp_and_workspace_bind() {
        let args = bubblewrap_profile_args(SandboxMode::WorkspaceWrite, Path::new("/workspace"))
            .into_iter()
            .map(|arg| arg.to_string_lossy().into_owned())
            .collect::<Vec<_>>();
        assert_eq!(
