...
impl std::fmt::Display for ToolRuntimeError
{
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result
    {
        match self
        {
            ...
            Self::DuplicateParameter { tool, parameter } => write!(
                f,
                "Duplicate parameter {parameter:?} in tool {tool:?}"
            ),
            ...
            Self::UndeclaredParameter { tool, parameter } => write!(
                f,
                "Undeclared parameter {parameter:?} for tool {tool:?}"
            ),
