...
impl Ord for CompactHeapCandidate {
    fn cmp(&self, other: &Self) -> Ordering {
        // `BinaryHeap` is a max-heap, so reverse the canonical priority. The
        // remaining fields establish a total deterministic ordering only after
        // `(rank, left-index)` is equal.
        other
            .priority
            .cmp(&self.priority)
