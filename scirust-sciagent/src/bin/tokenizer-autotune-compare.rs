...
#[command(
    ...
    about = "Compare exact-case ElasticTokenizer autotune reports"
)]
struct Args {
    ...
    /// Candidate raw report produced with the same tokenizer, cases and hardware.
    #[arg(long)]
    candidate: PathBuf,
}
...
fn main() {
    ...
    for key in baseline
        ...
        .chain(baseline.disqualified.iter())
        .chain(candidate.disqualified.iter())
        .cloned()
        .collect::<BTreeSet<_>>()
    {
        ...
        let speedup = match (baseline_median, candidate_median, status)
        {
            (Some(base), Some(candidate), "ok") if candidate != 0 => {
...
fn load_report(path: &Path) -> Result<RawReport, String> {
    ...
    if schema != u64::from(AUTOTUNE_REPORT_SCHEMA_V2)
    {
        return Err(format!("unsupported autotune report schema {schema}; expected v2"));
    }

    let tokenizer_fingerprint = required_str(&value, "tokenizer_fingerprint")?.to_string();
    let case_fingerprint = required_str(&value, "case_fingerprint")?.to_string();
    let hardware = value
        .get("hardware")
    ...
    let calibration = value
        .get("calibration")
        .ok_or_else(|| "autotune report missing `calibration`".to_string())?;
    let protocol = TimingProtocol {
        warmup_runs: usize::try_from(required_u64(calibration, "warmup_runs")?)
            .map_err(|_| "warmup_runs exceeds this platform".to_string())?,
        measured_runs: usize::try_from(required_u64(calibration, "measured_runs")?)
            .map_err(|_| "measured_runs exceeds this platform".to_string())?,
    };
...
fn verify_comparable(baseline: &RawReport, candidate: &RawReport) -> Result<(), String> {
    ...
    {
        return Err("exact calibration-case fingerprints differ".to_string());
    }
    ...
    {
        return Err("timing protocols differ".to_string());
    }
    Ok(())
}
...
mod tests {
    ...
    fn load_temp(name: &str, input: &str) -> RawReport {
    ...
    #[test]
    fn comparability_requires_same_tokenizer_cases_hardware_and_protocol() {
        let baseline = load_temp(
            "scirust_autotune_compare_v2_a.json",
            &report("tok", "cases", "hw", 2, 9, true),
        );
        let same = load_temp(
            "scirust_autotune_compare_v2_b.json",
            &report("tok", "cases", "hw", 2, 9, true),
        );
        verify_comparable(&baseline, &same).unwrap();

        for (name, other) in [
            ("tokenizer", report("other", "cases", "hw", 2, 9, true)),
            ("cases", report("tok", "other", "hw", 2, 9, true)),
            ("hardware", report("tok", "cases", "other", 2, 9, true)),
            ("protocol", report("tok", "cases", "hw", 3, 9, true)),
        ]
        {
            let candidate = load_temp(
                &format!("scirust_autotune_compare_v2_{name}.json"),
                &other,
            );
            assert!(verify_comparable(&baseline, &candidate).is_err(), "{name}");
        }
    }
    ...
    fn semantic_mismatch_disqualifies_kernel_length_pair() {
        let loaded = load_temp(
            "scirust_autotune_compare_v2_mismatch.json",
            &report("tok", "cases", "hw", 2, 9, false),
        );
        assert!(
            loaded
                .disqualified
    ...
    fn old_schema_is_rejected() {
        let path = std::env::temp_dir().join("scirust_autotune_compare_v1_rejected.json");
