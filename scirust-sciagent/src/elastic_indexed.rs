...
//! a wide compatibility path preserves semantics outside that domain.
...
impl IndexedBpe {
    ...
    fn encode_ids_compact(&self, input: &[TokenId]) -> Vec<TokenId> {
        ...
        while current != COMPACT_INDEX_NONE
        {
            ...
            output.push(
                usize::try_from(nodes[index].token).expect("u32 token id fits usize"),
            );
...
mod tests {
    ...
    fn compact_layout_reduces_working_set() {
        ...
        if usize::BITS == 64
        {
            ...
            assert!(
                std::mem::size_of::<CompactCandidate>() < std::mem::size_of::<WideCandidate>()
            );
        }
    }
