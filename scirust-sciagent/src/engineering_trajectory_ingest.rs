...
//!
//! P7.2 keeps the RSI v3 trajectory wire format as immutable provenance while
//! producing a SciAgent-native dataset view. Split assignment is grouped by the
//! frozen `task_spec_id`, so all trajectories for one held-out task stay out of
//! training. Deduplication happens before splitting and rejected trajectories
//! remain first-class negative examples.
...
impl EngineeringDataset {
    ...
    {
        config.validate()?;
        let mut by_state: BTreeMap<String, (String, EngineeringDatasetExample)> = BTreeMap::new();
        ...
        for record in records
        {
            ...
            match by_state.get(&example.semantic_state_id)
            {
                Some((source, _)) if source == &example.source_sha256 =>
                {
                ...
                Some((source, _)) =>
                {
                    return Err(EngineeringTrajectoryIngestError::ConflictingDuplicate {
                        ...
                        first_source_sha256: source.clone(),
    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self
        {
            ...
            Self::UnsupportedSchema(version) =>
            {
                write!(
                    f,
                    "unsupported engineering trajectory schema version: {version}"
                )
            },
...
fn example_from_trajectory(
    ...
    let patch_object = patch_set.as_object().ok_or_else(|| {
        EngineeringTrajectoryIngestError::InvalidRecord("patch_set must be an object".to_string())
    })?;
    let patch_set_identity = required_hex_identity(patch_object, "identity", &[64])?;
    let operations = patch_object
        .get("operations")
    ...
    if accepted
        && !all_hard_gates_pass(admissibility.as_object().ok_or_else(|| {
            EngineeringTrajectoryIngestError::InvalidRecord(
    ...
    let semantic_state_id =
        sha256_hex(format!("{task_spec_id}\n{parent_state_id}\n{patch_set_identity}").as_bytes());
...
fn split_for_task(task_spec_id: &str, config: &EngineeringIngestConfig) -> EngineeringDatasetSplit {
    ...
    let bucket =
        u32::from_be_bytes([digest[0], digest[1], digest[2], digest[3]]) % config.split_modulus;
...
fn canonical_json(value: &Value) -> Result<String, EngineeringTrajectoryIngestError> {
    serde_json::to_string(&canonical_value(value)).map_err(EngineeringTrajectoryIngestError::Json)
}
...
mod tests {
    ...
    fn trajectory(task: char, parent: char, patch: char, accepted: bool) -> String {
        let gate = if accepted { "pass" } else { "fail" };
        json!({
            "schema_version": 3,
            "task_spec_id": task.to_string().repeat(64),
            "compatibility": {
                "revisions": [{
                    "repository": "Memorithm/scirust",
                    "revision": "8c051f664fb82465215569d94fa640bf8d0328f3",
                    "role": "target"
                }],
                "toolchain": "1.89.0",
                "feature_contract": "default"
            },
            "parent_state_id": parent.to_string().repeat(64),
            "patch_set": {
                "identity": patch.to_string().repeat(64),
                "operations": [{
                    "kind": "create",
                    "path": "src/new.rs",
                    "content": "pub fn x() {}"
            ...
            "proposer": {
                "provider": "fixture",
                "model": "fixture-v1",
                "configuration_id": "deterministic"
            },
