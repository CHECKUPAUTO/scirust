...
- **`sos-cli`** — the `sos` command-line porcelain (RFC-0002 §10.4), the
  ...
  It binds exactly **one** plugin, and that limit is a consequence rather than
  a shortcut worth apologising for. `OdeStageHandler` integrates a Rust
  closure; no file can name a closure, so no amount of CLI plumbing could let
  a user supply one. Only a backend whose configuration is *data* —
  `CatalogStageHandler`'s `ModelRun` — can be driven from a command line at
  all. That is the practical payoff of giving models an identity, and the
  reason `ModelRun` gained an exact-decimal serde form here: a run has to
  survive a file with its content address intact or the manifest pinning it
  stops resolving.
