...
- **`sos-scirust`** — the Computational Backend Adapter, and the only other
  ...
  *measure* it rather than leaving it to prose: the relative dispersion across
  ...
  Stated precisely, because this is the item most easily overclaimed: `model`
  is a primitive, not a feature. No `StageExecutor` binds the catalogue to
  `sos-workflow` the way `stage` binds the closure-based ODE backend, and the
  CLI is connected to neither — **`sos run` is still not real**. What is true
  is narrower and checkable: a catalogued model can be specified, addressed,
  stored, integrated and reproduced. The catalogue is also a closed enum of
  nine models, so SOS runs exactly what `scirust-sim` ships; a third-party
  model still needs the out-of-process plugin transport of RFC-0002 §10.

  What remains deferred, not stubbed: Welch averaging and spectrograms within
  the signal family, a stage handler and CLI wiring for the catalogue, and
  registry-mediated resolution of any of these capabilities (`sos-scirust` is
  the in-process "Static Rust… the default" transport of RFC-0002 §10 §1, so
  direct construction is the expected shape until a caller needs to swap
  implementations).
